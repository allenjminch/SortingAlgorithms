...
public class RecursiveInsertionSort {
	...
	public static void main(String[] args) {
		...
		recursiveInsertionSort(array2);
	    recursiveInsertionSort(array3);
		recursiveInsertionSort(array4);
		recursiveInsertionSort(array5);
		recursiveInsertionSort(array6);
		recursiveInsertionSort(array7);
	...
	public static void recursiveInsertionSort(int[] a, int low, int high) {
		if (low < high) {
			recursiveInsertionSort(a, low + 1, high);
			int insertion = a[low];
			int i = low + 1;
			boolean inserting = false;
			while (i <= high && !inserting) 
			    if (a[i] < insertion) {
			    	a[i - 1] = a[i];
			    	i++;
			    } else {
